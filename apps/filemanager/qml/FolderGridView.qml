...
GridView {
    ...
    cellHeight: {
        var iconHeight = iconSize + (NemacUI.Units.fontMetrics.height * 2) + NemacUI.Units.largeSpacing * 2
        if (isDesktopView) {
    ...
    cellWidth: {
        var iconWidth = iconSize + NemacUI.Units.largeSpacing * 4
    ...
    function rectangleSelect(x, y, width, height) {
        ...
        var cWidth = control.cellWidth - (2 * NemacUI.Units.smallSpacing)
        var cHeight = control.cellHeight - (2 * NemacUI.Units.smallSpacing)
        ...
        for (var s = 0; s < stripes; s++) {
            for (var i = 0; i < perStripe; i++) {
                ...
                // expensive work.
                if (control.rubberBand.intersects(Qt.rect(itemX + NemacUI.Units.smallSpacing, itemY + NemacUI.Units.smallSpacing,
                    cWidth, cHeight))) {
        ...
        TextField {
            ...
            onTargetItemChanged: {
                if (targetItem != null) {
                    ...
                    width = targetItem.width - NemacUI.Units.smallSpacing
                    height = targetItem.labelArea.paintedHeight + NemacUI.Units.largeSpacing * 2
                    x = targetItem.x + Math.abs(Math.min(control.contentX, control.originX))
                    y = pos.y - NemacUI.Units.smallSpacing
