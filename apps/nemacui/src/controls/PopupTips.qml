...
Window {
    ...
    width: label.implicitWidth + NemacUI.Units.largeSpacing * 1.5
    height: label.implicitHeight + NemacUI.Units.largeSpacing * 1.5
    ...
    Rectangle {
        ...
        radius: windowHelper.compositing ? NemacUI.Theme.mediumRadius : 0
    ...
    function adjustCorrectLocation() {
        ...
        if (posX <= Screen.virtualX)
            posX = Screen.virtualX + NemacUI.Units.smallSpacing

        // top
        if (posY <= Screen.virtualY)
            posY = Screen.virtualY + NemacUI.Units.smallSpacing
