...
void NemacUI::initializeEngine(QQmlEngine *engine, const char *uri)
{
    Q_ASSERT(QLatin1String(uri) == QLatin1String("NemacUI"));
...
void NemacUI::registerTypes(const char *uri)
{
    Q_ASSERT(QLatin1String(uri) == QLatin1String("NemacUI"));

    qmlRegisterSingletonType<ThemeManager>("NemacUI.Core", 1, 0, "ThemeManager", [](QQmlEngine *engine, QJSEngine *scriptEngine) -> QObject * {
        Q_UNUSED(engine)
...
QUrl NemacUI::componentUrl(const QString &fileName) const
{
    return QUrl(QStringLiteral("qrc:/nemacui/kit/") + fileName);
}
