...
ThemeManager::ThemeManager(QObject *parent)
    : QObject(parent)
    , m_settings(new QSettings(QStringLiteral("nemacde"), QStringLiteral("theme")))
{
    ...
    m_wallpaperPath = m_settings->value("Wallpaper", "/usr/share/backgrounds/nemacde/default.jpg").toString();
    ...
    if (!m_settings->contains(s_systemFontName)) {
        QSettings lanSettings(QStringLiteral("nemacde"), QStringLiteral("language"));
...
void ThemeManager::setDevicePixelRatio(qreal ratio)
{
    ...
    QProcess p;
    p.setProgram("pkexec");
    p.setArguments(QStringList() << "nemac-sddm-helper"
...
void ThemeManager::updateGtk3Config()
{
    ...
    // theme
    settings.setValue("gtk-theme-name", isDarkMode() ? "Nemac-dark" : "Nemac-light");
    settings.sync();
}
...
void ThemeManager::updateFontConfig()
{
    ...
    QFile file(targetPath + "/99-nemac.conf");
