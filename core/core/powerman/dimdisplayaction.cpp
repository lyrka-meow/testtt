/***************************************************************************
 *   Copyright (C) 2021 by Reion Wong <reion@nemacde.com>               *
...
DimDisplayAction::DimDisplayAction(QObject *parent)
    : Action(parent)
    , m_iface("com.nemac.Settings",
              "/Brightness",
              "com.nemac.Brightness", QDBusConnection::sessionBus())
{
...
void DimDisplayAction::onIdleTimeout(int msec)
{
    ...
    if (sec == m_dimOnIdleTime) {
        ...
        // Sleep
        if (m_sleep) {
            QDBusInterface iface("com.nemac.Session",
                                 "/Session",
                                 "com.nemac.Session", QDBusConnection::sessionBus());
